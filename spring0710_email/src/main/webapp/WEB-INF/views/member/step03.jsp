...
<script type="text/javascript">
	function memberBtn() {
		// 자바스크립트 정규표현식
		let nameCheck = /^[가-힣]+$/; // ^:시작 []:조건 +$:1개이상끝까지
		let numCheck = /^[0-9]+$/; // 숫자 파악
		let alphaCheck = /^[a-zA-Z]+$/; // 영문자 파악
		let etcCheck = /^[!@#$%^&*,.?_-]+$/; // 특수문자 파악
		let idCheck = /^[a-zA-Z0-9_]{2,5}$/ // 아이디(영문+숫자+_, 2~5자릿수) 파악
		let idCheck2 = /^[a-zA-Z]{1}[a-zA-Z0-9_]{2,5}$/; // 첫째자리는 영문자만
		let pwCheck = /^(?=.*[a-zA-Z])(?=.*[0-9])(?=.*[!@#$%^&*]).{4,20}$/; // ?:비교 .:이하 한개씩 *:모두   => 1개라도 해당되면 true!
		let phoneCheck = /^[0-9]{2,3}-[0-9]{3,4}-[0-9]{4}$/ // 핸드폰 번호 파악
		
		
		
		/* if (!pwCheck.test($("#mem_pw").val())){
			alert("영문/숫자/특수문자가 1개이상 들어가야 합니다.");
			$("#mem_pw").focus;
			return false;
		} else {
			alert("성공");
		...
		/* if (!idCheck2.test($("#mem_id").val())){
			alert("2~5자리 영문/숫자/특수문자(_)만 입력가능");
			$("#mem_id").focus;
			return false;
		} else {
			alert("성공");
		...
		<div id="outbox">		
			...
			<div id="contents">
				<div id="member">
					...
					<form action="/member/step03" name="memberFrm" method="post"> 
						<div class="memberbd">
							<table summary="이름, 아이디, 비밀번호, 비밀번호 확인, 이메일, 이메일수신여부, 주소, 휴대폰, 유선전화, 생년월일 순으로 회원가입 정보를 등록할수 있습니다." class="memberWrite" border="1" cellspacing="0">
								...
								<tbody>
									<tr>
										...
										<td>
											<ul class="pta">
												...
												<li><span class="mvalign">※ 한글만 입력 가능 </span></li>
											</ul>
										</td>
									</tr>
									<tr>
									...
									<tr>
										...
										<td>
											<ul class="pta">
												<li class="r10"><input type="text" class="w134" name="pw" id="mem_pw"/></li>
									...
									<tr>
										...
										<td>
											<ul class="pta">
												<li class="r10"><input type="text" class="w134" onKeyUp="pwKey()" name="pw2" id="mem_pw2" /></li>
												<li id="pwKeyText">
												...
												<script>
													function email_chn() {
														...
														if ($("#emailList").val() == "txt") {
															...
															$("#email2").attr("readonly", true); /* attr: 속성값 */
														}
													}
												</script>
												<li>
						...
						<div class="btnArea">
							<div class="bCenter">
								<ul>
									...
									<li><a onclick="memberBtn()" style="cursor: pointer;" class="sbtnMini">가입하기</a></li>
								</ul>
							</div>
						</div>
						<!-- //Btn Area -->
