...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt"%><!-- formater -->
	...
	<section>
			...
			<form action="/board/boardList" name="search" method="post">
				...
				<div class="title">
					<input type="text" style="display: none;"> <input
						type="text" name="s_word" id="s_word" size="16" value="${s_word}">
				</div>
			...
			<c:forEach var="board" items="${list}">
				...
				<tr>
					...
					<td class="table-title"><c:forEach begin="1"
							end="${board.bindent}" step="1">
							<!-- 답변 들여쓰기(bindent는 답변 달릴 때마다 1씩 증가함) -->
							<img src="/images/icon_reply.png">
						</c:forEach> <a
						href="boardView?bno=${board.bno}&page=${page}&category=${category}&s_word=${s_word}">
							<!-- 검색범위와 검색어도 들고감  --> ${board.btitle}
					</a></td>
					<td>${board.id}</td>
					<td><fmt:formatDate value="${board.bdate}"
							pattern="yyyy-MM-dd" /> <!-- 날짜 형식 지정 --></td>
					<td>${board.bhit}</td>
				</tr>
			</c:forEach>
		...
		<ul class="page-num">
			...
			<c:if test="${page != startPage}">
				<a
					href="/board/boardList?page=1&category=${category}&s_word=${s_word}"><li
					class="first"></li></a>
			...
			<c:if test="${page > 1}">
				<a
					href="/board/boardList?page=${page-1}&category=${category}&s_word=${s_word}"><li
					class="prev"></li></a>
			</c:if>
			<c:if test="${page == 1}">
				<li class="prev"></li>
			...
			<c:forEach begin="${startPage}" end="${endPage}" step="1" var="num">
				<c:if test="${num != page}">
					<a
						href="/board/boardList?page=${num}&category=${category}&s_word=${s_word}">
			...
			<c:if test="${page != maxPage}">
				<a
					href="/board/boardList?page=${maxPage}&category=${category}&s_word=${s_word}"><li
					class="last"></li></a>
			</c:if>
			<c:if test="${page == maxPage}">
				<li class="last"></li>
