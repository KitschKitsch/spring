...
<body>
	<section>
		...
		<table>
			...
			<tr>
				<td colspan="3" class="article"><strong>파일명</strong> <span
					class="separator">|</span>${bdto.bfile}</td>
			</tr>
			<tr>
				<td colspan="3"><c:if test="${bdto.bfile != null}">
						<img src="/images/${bdto.bfile}">
					</c:if> <c:if test="${bdto.bfile == null}">
		...
		<a href="boardList?page=${page}&category=${category}&s_word=${s_word}"><div
				class="list">목록</div></a>
		<!-- 이전 검색 페이지로 -->
		<a onclick="deleteBtn()"><div class="list">삭제</div></a> <a
			href="boardUpdate?bno=${bdto.bno}&page=${page}&category=${category}&s_word=${s_word}"><div
				class="list"">수정</div></a> <a
			href="boardReply?bno=${bdto.bno}&page=${page}&category=${category}&s_word=${s_word}"><div
				class="list"">답변달기</div></a>
		<!-- 참고) @PathVariable: dbno=1  =>  /1로 단순하게! -->
	</section>
</body>
</html>